...
public class ProxyController {
	...
	public void onPluginMessage(byte[] data, String serverName, ImmersiveChatProxySource source) {
		...
		try {
			...
			if(subchannel.equals("Chat")) {
				...
				for(ImmersiveChatProxyServer send : source.getServers()) {
					if(!send.isEmpty()) {
			...
			if(subchannel.equals("Chwho")) {
				...
				if(identifier.equals("Get")) {
					...
					SynchronizedImmersiveChatPlayer smcp = playerApiService.getSynchronizedMineverseChatPlayer(UUID.fromString(sender));
					if(smcp == null) {
			...
			if(subchannel.equals("Sync")) {
				...
				if(identifier.equals("Receive")) {
					...
					SynchronizedImmersiveChatPlayer smcp = playerApiService.getSynchronizedMineverseChatPlayer(uuid);
					if(smcp == null) {
						smcp = SynchronizedImmersiveChatPlayer.builder().uuid(uuid).build();
